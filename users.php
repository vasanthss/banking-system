 ...
 if($amnt > $sql1['amount'])
    {
        ...
        $newCredit = $sql1['amount'] - $amount;
        $sql = "UPDATE customer set amount=$newCredit where id=$from";
        mysqli_query($conn,$sql);



        $newCredit = $sql2['amount'] + $amount;
        $sql = "UPDATE customer set amount=$newCredit where id=$receiver";
        mysqli_query($conn,$sql);
...
<head>
    ...
    <title>Vasanth Basic banking system</title>
...
<body>
  ...
  <p class="logo-text">Sukitha Banking system</p>
